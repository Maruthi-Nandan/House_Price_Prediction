...
class ZScoreOutlierDetection(OutlierDetectionStrategy):
    ...
    def detect_outliers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logging.info(f"Outliers detect with Z-score threshold: {self.threshold}")
        return outliers
    
class IQROutliersDetection(OutlierDetectionStrategy):
    def detect_outliers(self, df: pd.DataFrame) -> pd.DataFrame:
        logging.info("Detected outliers using the IQR method.")
        Q1 = df.quantile(0.25)
        Q3 = df.quantile(0.75)
        IQR = Q3 - Q1
        outliers = (df < (Q1 - 1.5*IQR))|(df > (Q3 + 1.5*IQR))
        logging.info("Outliers detected using the IQR")
        return outliers
